...
# MINI EXERCISE 1 RESPONSE
```{r}
tr_primates

str("tr_primates")
```
# str() provides the general information about the tree, including tip labels and branch length. 
```{r}
str(tr_primates)
...
names(tr_primates) 
```
# attributes() and names() provide the attributes and names of the species. 
#ERRONEOUS TREE
```{r}
...
```{r}
newick_str = "(((Pinus, Picea), Larix), Abies);"
tr_plants = read.tree(text=newick_str)
plot(tr_plants)
```
...
```{r}
newick_str = "((((Pinus, Picea), Larix), Abies),(Radish),(Seqouia));"
tr3_plants = read.tree(text=newick_str)
plot(tr3_plants)
```
```{r}
newick_str = "(((Pinus, Picea),(Seqouia,Larix), Abies),(Radish));"
tr4_plants = read.tree(text=newick_str)
plot(tr4_plants)
```
a. Explain what's wrong in your erroneous tree and how you've corrected the error in the other tree.
# The erroneous error is that I placed Sequoia with Larix, when it should be on a separate branch on its own. I corrected this in my corrected version by putting in a separate parenthetical bracket. 
...
attributes(tr4_plants)
```
# the trees have the same numbers of tips, however the second tree has 1 fewer node than the first. This also results in one more edge in the first tree. 
# END MINI EXERCISE 2
...
```{r}
newick_str = "(((Humans:12.5, Chimps:12.5):1.0, Gorillas:14.0):1.0, (Pongo abelii:10.5, Pongo tapanuliensis:10.5)):1.0;"
tr_primate = read.tree(text=newick_str)
plot(tr_primate)
```
...
```{r}
newick_str = "(Sequoia, Wheat,(Corn,(Cabbage, Kale, Radish)));"
tr2_plants = read.tree(text=newick_str)
plot(tr2_plants)
```
...
taxa
tree_phylomatic_Arto <- phylomatic(taxa = Arto, get = 'POST')
plot(tree_phylomatic_Arto, no.margin = TRUE, type= "phylogram")
newick_fn2 = "tree_phylomatic_Arto"
write.tree(tree_phylomatic_Arto, file=newick_fn2)
```
...
# The differences between the cladograms and the phylograms is that there is a higher level of organization of the species. The phylograms include evolutionary time and character changes throughout time, while the cladograms do not. 
...
# It was difficult to include these taxa because of lack of knowledge regarding various levels of classification. 
